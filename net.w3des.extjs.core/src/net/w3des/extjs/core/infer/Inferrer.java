...
public class Inferrer extends ASTVisitor {
	...
	public boolean visit(IFunctionCall functionCall) {
		...
		if (mess.getReceiver() == null && mess.getSelector() != null && String.valueOf(mess.getSelector()).equals("alias")) {
			mess.resolvedType = TypeBinding.wellKnownType(null, TypeIds.T_function);
			mess.expectedType = TypeBinding.wellKnownType(null, TypeIds.T_function); 
			return true;
		}
	...
	public boolean visit(ICompoundAssignment compoundAssignment) {
		return super.visit(compoundAssignment);
	}
	...
	private void create(MessageSend mess) {
		if (mess.getArguments().length > 0 && mess.getArguments()[0] instanceof StringLiteral) {
			...
			if (tt == null) {
				tt = unit.addType(String.valueOf(className.source()).toCharArray(), false, InferProvider.ID); //find another method
	...
	private void define(MessageSend mess) {
		...
		if ((mess.getArguments()[0] instanceof StringLiteral)) {
			...
			newType = unit.addType(name.source(), true, InferProvider.ID); 
			...
			newType = unit.addType(InferredType.FUNCTION_NAME, true, InferProvider.ID);
	...
	private void aliases(StringLiteral alias) {
		InferredType newAlias = unit.addType(alias.source(), true, InferProvider.ID);
	...
	public boolean visit(IAssignment assignment) {
		...
		if (assignment.getLeftHandSide().toString().contains("Ext.")) {
			if (!imported) {
				imported = true;
				unit.addImport("Ext".toCharArray(), 0, 0, 0);
			}
		}
		...
		if (assignment.toString().equals("Ext.Base = Base")) { //simple hack to register Ext.Base
			...
			InferredType type = unit.addType("Ext.Base".toCharArray(), true, InferProvider.ID);
